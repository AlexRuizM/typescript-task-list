...
<td>
  ...
  //Añadir nueva tarea
  addTask = (input: HTMLInputElement | null, idGenerator: () => string, text: string, alert: HTMLElement | null): void => {
    if (input?.value.trim() === "") {
      input.value = "";
      ...
      document.querySelector("tbody")?.appendChild(this.generateRow(id, text, alert?.classList.contains("dismissible") ?? false));
      input!.value = "";
    }
  };
  //Modo Edición
